...
TEST_CASE( "type-specific sorters with Schwartzian transform adapter", "[schwartz_adapter]" )
{
    ...
    for (const auto& str: collection3) {
        collection4.emplace_back(str.value);
    }
    ...
    std::shuffle(collection4.begin(), collection4.end(), engine);
    ...
    SECTION( "ska_sorter" )
    {
        ...
        sorter(collection4, &wrapper<std::string_view>::value);
        CHECK( helpers::is_sorted(collection4.begin(), collection4.end(),
    ...
    SECTION( "spread_sorter" )
    {
        ...
        sorter(collection4, std::greater<>{}, &wrapper<std::string_view>::value);
