           ...
           "[utility][as_projection]" )
{
    ...
    SECTION( "without an untransformed function" )
    {
        ...
        vec = collection;
        cppsort::stable_adapter(cppsort::pdq_sort)(vec, func);
        CHECK( std::is_sorted(std::begin(vec), std::end(vec), std::greater<>{}) );

        vec = collection;
        cppsort::stable_adapter(cppsort::pdq_sort)(std::begin(vec), std::end(vec), func);
        CHECK( std::is_sorted(std::begin(vec), std::end(vec), std::greater<>{}) );
    }
    ...
    SECTION( "with a function wrapped in as_projection" )
    {
        ...
        vec = collection;
        cppsort::stable_adapter(cppsort::pdq_sort)(vec, cppsort::utility::as_projection(func));
        CHECK( std::is_sorted(std::begin(vec), std::end(vec)) );

        vec = collection;
        cppsort::stable_adapter(cppsort::pdq_sort)(std::begin(vec), std::end(vec), cppsort::utility::as_projection(func));
        CHECK( std::is_sorted(std::begin(vec), std::end(vec)) );
    }
    ...
    SECTION( "with a function wrapped in as_comparison" )
    {
        ...
        vec = collection;
        cppsort::stable_adapter(cppsort::pdq_sort)(vec, cppsort::utility::as_comparison(func));
        CHECK( std::is_sorted(std::begin(vec), std::end(vec), std::greater<>{}) );

        vec = collection;
        cppsort::stable_adapter(cppsort::pdq_sort)(std::begin(vec), std::end(vec), cppsort::utility::as_comparison(func));
        CHECK( std::is_sorted(std::begin(vec), std::end(vec), std::greater<>{}) );
    }
}
